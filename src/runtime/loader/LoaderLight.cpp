...
std::string LoaderLight::generateFinite(ShadingTree& tree)
{
    ...
    //initailize a 4D histogram for all lights
    stream << "let lights4DHisto = histogram::make_histo_4d(device, " << mFiniteLights.size() << ", 8, 8, 8, 8);" << std::endl;
...
std::string LoaderLight::generateGuidedLightSelector(std::string type, ShadingTree& tree)
{
    const std::string uniformSelector = "  let light_selector = make_uniform_light_selector(infinite_lights, finite_lights);";
    ...
    stream <<"  let light_histogram = if settings.iter == 0 {histogram::make_histo_1d(device, finite_lights.count)} else {histogram::normalize_histo_1d(device, finite_lights.count)};" << std::endl
           << "  let light_selector = make_histo_light_selector(infinite_lights, finite_lights, light_histogram);" << std::endl;
